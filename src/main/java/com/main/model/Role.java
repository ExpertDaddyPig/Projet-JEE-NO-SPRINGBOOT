...
public enum Role {
    ...
    public static Role fromLevel(int level) { // ⭐ Le nom de la méthode est en camelCase
        for (Role role : Role.values()) {
            if (role.level == level) { // ⭐ Utilisez role.level car c'est un champ privé
                return role;
            }
        }
        return EMPLOYE; // Fallback par défaut
    }
    /**
