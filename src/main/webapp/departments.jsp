            ...
            <body>
                ...
                <div class="container">
                    ...
                    <c:if test="${param.success eq 'add'}">
                        <div style="color:green; margin-bottom:10px;">Ajouté avec succès.</div>
                    </c:if>
                    <c:if test="${param.success eq 'update'}">
                        <div style="color:blue; margin-bottom:10px;">Mis à jour avec succès.</div>
                    </c:if>
                    <c:if test="${param.success eq 'delete'}">
                        <div style="color:red; margin-bottom:10px;">Supprimé avec succès.</div>
                    </c:if>
                            ...
                            <tbody>
                                ...
                                employees: '${dept.employees}' 
                                            ...
                                            <button class="btn-icon btn-delete"
                                                onclick="openDeleteModal('${dept.id}')">🗑</button>
                                        </td>
                                    </tr>
                                </c:forEach>
                            </tbody>
                    ...
                    <div class="modal-content">
                        ...
                        <form id="deptForm" method="post">
                            ...
                            <div class="form-group">
                                ...
                                <!-- SELECT MULTIPLE -->
                                <select class="form-control" name="employeeIds" id="deptEmployees" multiple size="6"
                                    style="height: auto;">
                                    <c:forEach var="emp" items="${allEmployees}">
                                        <option value="${emp.id}">${emp.first_name} ${emp.last_name} (${emp.job_name})
                                        </option>
                                    </c:forEach>
                                </select>
                                <small style="color: #888; font-size: 12px;">Maintenir Ctrl (Windows) ou Cmd (Mac) pour
                                    sélectionner plusieurs.</small>
                            </div>

                            <div class="modal-footer">
                                <button type="button" class="btn-logout" style="background:#6c757d;"
                ...
                <div id="deleteModal" class="modal-overlay">
                    <div class="modal-content" style="max-width: 400px; text-align: center;">
                        ...
                        <form action="${pageContext.request.contextPath}/departments/delete" method="post"
                            ...
                            <button type="button" class="btn-logout" style="background: #6c757d;"
                                onclick="document.getElementById('deleteModal').style.display='none'">Annuler</button>
                            <button type="submit" class="btn-delete"
                                style="padding: 8px 16px; border-radius: 5px;">Supprimer</button>
                        </form>
                    </div>
                </div>
